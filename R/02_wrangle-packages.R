...
inst_tbl <- read_csv(here("data", "installed-packages.csv"))

## filter out the base and recommended packages
## keep the variables Package and Built
## if you use dplyr, code like this will work:
non_base_tbl <- inst_tbl %>%
...
## if you use dplyr, code like this will work:
non_base_freqtable <- non_base_tbl %>%
  count(Built) %>%
  mutate(prop = n / sum(n)) %>%
  arrange(desc(Built))
...
## hint: readr::write_csv() or write.table()
## idea: try using here::here() to create the file path
write_csv(non_base_freqtable, here("data", "add-on-packages-freqtable.csv"))
